...
**Revision 2 — static art target:** the blocky procedural scene was rejected. The review now displays a generated environment plate with irregular cultivated plots, orchard margins, a worn cart track, bridge, sluice and farmstead. Building design, plant species, clothing and exact water layout are proposals, not canon. Green follows cultivation and water. Avoid transforming farmland into wild jungle or putting continuous red dunes through it. This is not a playable environment upgrade; layered assets, collision and animation remain to be built. Full contract and production translation: `ATALUK_ART_TARGET.md` (source repository). The user subsequently rejected the European architectural feeling. A separate playable exterior proof (`?view=ataluk-play`) now uses a proposed African-influenced earthen courtyard compound, terrace, grain store and woven shade. It has live character movement and depth occlusion, but remains below the picture’s visual quality. See `ATALUK_PLAYABLE_PROOF.md`; the cottage in the earlier image is superseded.
...
Raha coast: the painted open gate, vaulted craft house, shell counter, net frame and small fishing hull are a local shore layout grounded in the same Raha passages. Exact building construction, vessel type and street positions are proposals. The anonymous shore worker idles; dialogue, net repair, fishing and trade are not implemented by this slice.
